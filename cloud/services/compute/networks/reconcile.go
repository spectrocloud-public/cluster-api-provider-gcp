...
func (s *Service) Delete(ctx context.Context) error {
	...
	// Delete routes associated with network (Spectro fork behavior: GCP does not
	// auto-clean k8s-node-route entries when the network is deleted, which orphans
	// them and blocks a same-name network from being recreated cleanly).
